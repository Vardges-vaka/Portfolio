...
export function useForceGraph({ graph, cfg, isDark, tune, groups, subs, isVisible, labels, onPick }) {
  ...
  useEffect(() => {
    ...
    simRef.current = sim;
    recomputeActive();
    let userMoved = false; // once the visitor pans/zooms, stop auto-fitting
    ...
    // keep the graph framed as it settles, until the visitor takes over
    const onTick = () => {
      if (userMoved) draw();
      else dataRef.current.fit(1); // zoom-out-only auto-fit (draws)
    };
    if (reduced) {
      sim.stop();
      for (let i = 0; i < 320; i++) sim.tick();
      drawBackdrop();
      // initial foreground fit happens at the end of the effect (fit() defined below)
    } else {
      sim.on("tick", onTick);
    ...
    const onDown = (e) => {
      ...
      userMoved = true; // visitor is interacting — hand the camera over to them
    ...
    const ro = new ResizeObserver(() => {
      ...
      if (reduced) {
        ...
        sim.alpha(0.3).restart(); // re-settles → onTick re-fits (unless moved)
      }
    });
    ro.observe(wrap);
    ...
    // Fit the graph (incl. the always-on skill labels) into the stage. maxK caps
    // how far it may zoom IN — the auto-fit passes 1 (zoom-out only, so a wide
    // screen is never blown up); the manual fit button passes the default.
    dataRef.current.fit = (maxK = 2) => {
      const aNodes = dataRef.current.activeNodes || nodes;
      ...
      const calcK = (gw, gh) =>
        Math.max(0.35, Math.min(maxK, Math.min((w - 56) / (gw || 1), (h - 56) / (gh || 1))));
      let k = calcK(maxX - minX, maxY - minY);
      // Skill labels are centered under their node at a fixed ~12px screen size,
      // so their world half-width is screenWidth / k. Expand the box to include
      // them so they never clip on the right/left; two passes converge on k.
      for (let pass = 0; pass < 2; pass++) {
        aNodes.forEach((n) => {
          if (n.type !== "skill") return;
          const text = lbl[n.id] ?? n.label;
          if (!text) return;
          ctx.font = "600 12px Manrope, system-ui, sans-serif";
          const halfW = ctx.measureText(text).width / 2 / k;
          minX = Math.min(minX, n.x - halfW);
          maxX = Math.max(maxX, n.x + halfW);
          maxY = Math.max(maxY, n.y + effR(n) + 18 / k);
        });
        k = calcK(maxX - minX, maxY - minY);
      }
      const tf = tfRef.current;
      tf.k = k;
    ...
    // initial framing for reduced-motion (no tick loop to drive onTick)
    if (reduced) dataRef.current.fit(1);
