  ...
  return (
    <Motion.article
      ...
      <div className={`vp-cert-card__flip ${flipped ? "is-flipped" : ""}`}>
        {/* FRONT */}
        <div
          className="vp-cert-card__face vp-cert-card__face--front"
          aria-hidden={flipped}>
          ...
          <div className="vp-cert-card__foot">
            ...
            {canFlip && (
              <button
                type="button"
                className="vp-cert-card__flip-btn"
        ...
        {canFlip && (
          <div
            className="vp-cert-card__face vp-cert-card__face--back"
            aria-hidden={!flipped}>
            <span className="vp-cert-card__org">
              <Award size={13} aria-hidden="true" />
              {cert.org}
            ...
            <button
              type="button"
              className="vp-cert-card__flip-btn vp-cert-card__flip-btn--back"
...
const CertWall = ({
  ...
  const filtered =
    active === "all" ? rest : rest.filter((c) => c.cat === active);
  const countFor = (key) =>
    key === "all" ? rest.length : rest.filter((c) => c.cat === key).length;
      ...
      {collapsible && rest.length > 0 && (
        <div className="vp-showall">
          {fullWall ? (
            <button
              type="button"
              className="vp-btn vp-showall__btn"
              onClick={collapse}>
            ...
            <>
              {!allShown && (
                <button
                  ...
                  onClick={() =>
                    setVisible((v) => Math.min(rest.length, v + step))
                  }>
                  <Plus size={15} aria-hidden="true" />
                  {loadMoreLabel}
                  <span className="vp-showall__num">
